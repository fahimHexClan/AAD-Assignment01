...
searchField.on('input', function () {
    ...
    $('tbody').eq(1).empty();        //search karapu fild eke value show karanna table eka empty karanawa
...
submit.on('click', (e) => {
    ...
    let itemNameValue = itemName.val().trim();//trim eken unwanted spaces remove karanawa
    let priceValue = parseFloat(price.val());//ganna value eka float karagannawa
    let qtyOnHandValue = parseInt(qtyOnHand.val(), 10);//meya int  karagena 10 use karanne  radix ekak->(base-10 )
...
$('#itemTable').on('click', 'tbody tr', function() {
    ...
    let qtyOnHandValue = $(this).find('td:eq(2)').text();//find eken me eliment ekata aadala data tika set karanawa

    itemCode.val(itemCodeValue);
    itemName.val(itemNameValue);//uda varible ekata set karapu eka input field dala
...
update.on('click', () => {
    ...
    if(
        ...
        validation(qtyOnHandValue, "Qty On Hand",null)){
            ...
            //map eken wenne db eke thiyana ekai dan ekai match karala update karanawa 
        item_db.map((item) => {
...
delete_btn.on('click', () => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            item_db.splice(index, 1);//splice eke 1st index select karala  remove coun eka danawa
            populateItemTable();
            resetColumns();
            Swal.fire(
                'Deleted!',
