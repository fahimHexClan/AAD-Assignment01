import {customer_db, item_db} from "../db/db.js"; //import customer db and itemdb array 
import {CustomerModel} from "../model/customerModel.js";//import customer model

let submit = $('#Customer .btn-success').eq(0);
let update = $('#Customer .btn-primary').eq(0);
let delete_btn = $('#Customer .btn-danger').eq(0);
let reset = $('#Customer .btn-warning').eq(0);//varible assign (eq(0)select the  first element in a jQuery object )
...
//customer search
searchField.on('input', function () {//event listener 
    let search_term = searchField.val();//srch eke value eka search_termekata assign kanawa

    let results = customer_db.filter((item) =>// result eka choose karanna use kanna ekak filter customer aaray eke

            //to lower case using for - Converts a string to lowercase.(searching for "abc" should match "ABC", "Abc", "aBc", etc.)
        ...
        //check sech fld same or no

    );

        //search karapu fild eke value show karanna table eka empty karanawa
    $('tbody').eq(0).empty();
    
    //meken karanne empty karapu table ekata  values add karanawa row ekata
...
function generateCustomerId() {
    ...
    for (let i = 0; i < customer_db.length; i++) {
        const numericPart = parseInt(customer_db[i].customer_id.split('-')[1]);//split eken num ekai - pace thiyala daganna eka wenne
...
//page eka load wenakota me tika load wenna
...
submit.on('click', (e) => {
    ...
    let addressValue = address.val().trim();// trim eken wenne whide space remove karanawa val eke - " ggg "-use trim ("ggg")
    ...
    if(
        ...
        customer_db.push(customer);//gattha data tika customer array ekata danawa
...
/*gaththa data tika table ekata set karanawa*/
...
// tbl eke thiyana row eka click karama input flds wlta set wenna 
$('#customerTable').on('click', 'tbody tr', function() {
    ...
    let emailValue = $(this).find('td:eq(3)').text();//find eken me eliment ekata aadala data tika set karanawa

    customer_id.val(customerIdValue);//uda varible ekata set karapu eka input field dala
    name.val(nameValue);
    address.val(addressValue);
    contact.val(contactValue);
    email.val(emailValue);

    submit.prop("disabled", true);//methana karanne  tble eke click karama tble  submit btn eka disable wena
    delete_btn.prop("disabled", false);//methana false karama weda
    update.prop("disabled", false);//methana false karama weda
...
delete_btn.on('click', () => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            customer_db.splice(index, 1);//splice eke 1st index select karala  remove coun eka danawa
            populateCustomerTable();
            resetColumns();
            Swal.fire(
                'Deleted!',
